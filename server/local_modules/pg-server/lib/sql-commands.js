...
const parseSorter = string => {
  ...
  if (result) {
      let [match, field, direction] = result;
      return {
          field,
          direction
      };
  } else {
      throw `Malformed sorter '${string}'`;
  }
};
...
Handlebars.registerHelper('value', function (value) {
  if (value === undefined) throw 'Missing value detected';
...
class From {
  constructor (definition) {
      ...
      if (table) {
          ...
          throw 'Expected a table in the from statement, but no table was provided';
      }
  }
  static parse (string) {
      let result = fieldRegEx.exec(string.trim());
      if (result) {
          let [match, schema, table, alias] = result;
          return {
              schema,
              table,
              alias
  ...
  getFields () {
      ...
      if (fields) {
          return fields.map(col => {
              ...
              if (col) {
                  let [match, table, field, fieldAlias] = col;
                  field = `${alias}.${field}`;
                  if (fieldAlias) {
                      field += ` AS ${fieldAlias}`;
                  }
  ...
  resolveTable (incoming) {
      ...
      let sql = table.resolve(incoming);
      if (schema) {
          sql = `${schema}.${sql}`;
      }
  ...
  // resolveAlias (incoming) {
  //     return this.alias || Select.getNextAlias(incoming);
  // }
  resolve (incoming) {
...
class Query {
  constructor (definition) {
      if (!definition) {
          throw 'Queries require a definition to be instantiated';
      }
...
class Select extends Query {
  prepare (definition) {
      ...
      this._aliasNum = 0;
      let {from, with: withClause, joins, fields, where} = this;
      let i, len;
      this.fields = fields = new Fields(fields);
      if (from) {
          this.from = from = new From(from);
          if (!from.alias) {
              from.alias = this.nextAlias();
          }
          fields.addAll(from.getFields());
      }
      if (joins) {
          let join;
          this.joins = [];
          for (i = 0, len = joins.length; i < len; i++) {
              this.joins[i] = join = new Join(joins[i]);
              if (!join.alias) {
  ...
  resolve (incoming) {
      let {from, with: withClause, distinct, joins, fields, where, groupBy, orderBy} = this;
      let sql = `SELECT`;
      // let fromClause = [];
      // let fields = [];
      // let i, len;
  ...
  static prepareTable (table) {
      ...
      } else if (table instanceof Query || table instanceof Relation) {
          return table;
      } else {
  ...
  //     return join;
  // }
  static getNextAlias (incoming) {
      return `_t${++incoming.t}`;
  }
}
